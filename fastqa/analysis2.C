...
//==================
//==================
int analysis2(int run, Long64_t totalevents=100000, float UST_thr=400) {
  ...
  TString outputpdf = Form("analysis2_Run%d.pdf",run);
  ...
  for(;nev!=nentries;++nev) {
    ...
    for(int idx=0; idx!=nchn; ++idx) {
      ...
      maxamp[ idx ] = getmin( trace[ chnidx[ idx ] ], maxtime[idx] );
    }
    ...
    //CUTS
    if(maxtime[1]<40 || maxtime[1]>90) continue;
    if(maxtime[2]<40 || maxtime[2]>90) continue;

    if(maxtime[5]<40 || maxtime[5]>100) continue;
    if(maxtime[6]<40 || maxtime[6]>100) continue;
    if(maxtime[7]<40 || maxtime[7]>100) continue;
