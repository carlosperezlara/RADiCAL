...
void DT5742DATReader::ReadHeader() {
  ...
  for(int ich=0;ich!=fNumberOfChannels;++ich) {
...
bool DT5742DATReader::ReadGroup(int iGroup) {
  ...
  if(TR) {
	...
	fTrace[16]->SetBinContent(real_isa+1,mV);
	fAll[16]->Fill(real_isa,mV);
	fTrace[17]->SetBinContent(real_isa+1,mV);
	fAll[17]->Fill(real_isa,mV);
      }
    }
  }
